...
m_inla_bin <- fit_inla3(dat,
  response = "present", n_knots = 40,
  family = "binomial"
)

dpos <- filter(dat, present == 1)
dpos$density <- dpos$density * 1e3
m_inla_pos <- fit_inla3(dpos,
  response = "density", n_knots = 40,
  family = "gamma"
)
```
...
yr_est_summ <- group_by(yr_est, year) %>%
  summarise(
    lwr = quantile(value * 1e3 * square, probs = 0.025),
    upr = quantile(value * 1e3 * square, probs = 0.975),
    biomass = quantile(value * 1e3 * square, probs = 0.5)
  ) %>%
